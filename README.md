...
- `卸载Cookie.command`：停止并把程序、状态与日志移到废纸篓（可恢复）。

## 她会做什么

- 在当前显示器底部散步，走到边缘会压低身体、转向再继续。
- 发呆时呼吸、摇尾巴，偶尔向左或向右看。
- 连续 45 秒没有鼠标移动就睡觉；睡着时三颗 Z 会依次上飘、长大再淡出，鼠标回来会醒。
- 连续使用电脑约 45 分钟，她会站起来叫你一起伸懒腰；离开够久后计时重新开始。
- 每 30 秒只读取一次系统内存压力；连续三次高压才会趴下说“电脑也累啦”，瞬时尖峰不误触发。
- Apple Mail 已经打开时，只读取 Inbox 的未读总数。数字增加时她会叼一封小信来；不读取标题、发件人、正文或账号凭据。
- 偶尔在鼠标靠近桌面底部时跑过去“啃”两口，只表演靠近和气泡，不移动、不点击、不接管鼠标。
- 只读取当前鼠标坐标来判断“有没有动”，不记录轨迹、不监听键盘或点击内容。
- 使用 190×214 的预缩放透明图层和 24 fps 渲染，适合小尺寸常驻。

慢速走路的横坐标使用独立的小数累加器。不要在每帧从 `NSWindow.frame()` 读取取整后的坐标再加小于 1px 的步长，否则小数位会被反复丢弃，Cookie 会只原地踏步。
...
四条腿的切片在转轴上方只保留很短的隐藏搭接，并始终锁在各自的 `x0..x1` 边界。若向上抓到胸毛或左右吃进邻脚，静止重组看不出问题，但转腿时会露出鬼影，表现成脚趾落地分叉。
